...
from DevBot import INSPECTOR, LOGGER, REQUESTER, dispatcher
from DevBot.Handlers.validation import user_admin, user_not_admin, is_user_admin
from DevBot.Plugins.Admin.log_channel import loggable
from DevBot.Database import reporting_sql as sql
    ...
    if chat.type == chat.PRIVATE:
        if len(args) >= 1:
            if args[0] in ("yes", "on"):
                ...
                msg.reply_text(
                    "Turned on reporting! You'll be notified whenever anDevBot reports something."
                )
...
def report(update: Update, context: CallbackContext) -> str:
    ...
    if chat and message.reply_to_message and sql.chat_should_report(chat.id):
        ...
        keyboardmark = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
        ...
        message.reply_text(
            f"{mention_html(user.id, user.first_name)} reported the message to the admins.", reply_markup=keyboardmark,
            parse_mode=ParseMode.HTML,
        )
        return msg
