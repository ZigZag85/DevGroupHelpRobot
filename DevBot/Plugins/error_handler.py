...
from DevBot import dispatcher, DEV_USERS, OWNER_ID, API_HASH, API_ID
...
def error_callback(update: Update, context: CallbackContext):
    ...
    e = e.replace(context.bot.token, "$TOKEN").replace(str(API_ID), "$API_ID").replace(API_HASH, "$API_HASH") #.replace(SPAMWATCH_API, "$SPAMWATCH_API")
    if not key:
