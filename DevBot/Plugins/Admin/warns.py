...
import telegram
from DevBot import INSPECTOR, REQUESTER, dispatcher
...
from DevBot.Handlers.extraction import (
    extract_text,
    extract_user,
...
from DevBot.Handlers.string_handling import split_quotes
from DevBot.Plugins.Admin.log_channel import loggable
from DevBot.Database import warns_sql as sql
...
def __stats__():
    return (
        f"• {sql.num_warns()} overall warns in {sql.num_warn_chats()} groups.\n"
        f"• {sql.num_warn_filters()} warn filters in {sql.num_warn_filter_chats()} groups."
    )
