...
from DevBot.Handlers.extraction import extract_user_and_text
from DevBot.Handlers.string_handling import extract_time
from DevBot.Plugins.Admin.log_channel import gloggable, loggable
...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            ...
            message.reply_text(
                "Bring an order from DevBot Management to fight a Requester."
            )
        else:
    ...
    try:
        ...
        bot.sendMessage(chat.id, reply, reply_markup = unban_keyboard, parse_mode=ParseMode.HTML)
        return log
...
def button(update: Update, context: CallbackContext) -> str:
    ...
    if match:
        ...
        if unabnned:
            ...
            update.effective_message.edit_text(
                "User already unbanned.", parse_mode=ParseMode.HTML
            )
...
def punch(update: Update, context: CallbackContext) -> str:
    ...
    if res:
        ...
        bot.sendMessage(
            chat.id,
            f"Kicked User! {mention_html(member.user.id, html.escape(member.user.first_name))} From Group.",
            parse_mode=ParseMode.HTML,
        )
        log = (
            f"<b>{html.escape(chat.title)}:</b>\n"
