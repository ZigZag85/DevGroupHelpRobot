from DevBot.Handlers.telethon import IMMUNE_USERS, telethn
from DevBot import REQUESTER
...
async def user_is_admin(user_id: int, message):
    ...
    async for user in telethn.iter_participants(
        ...
        if user_id == user.id or user_id in REQUESTER:
            status = True
            break
    return status
...
async def DevBot_is_admin(chat_id: int):
    status = False
    DevBot = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if DevBot.id == user.id:
            status = True
            break
    return status
