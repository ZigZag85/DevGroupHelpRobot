...
from DevBot.Handlers.validation import is_user_admin
...
from DevBot.Plugins import ALL_MODULES
from DevBot.__help__ import (
get_help, 
help_button, 
get_settings, 
settings_button, 
...
def start(update: Update, context: CallbackContext):
    ...
    [
        ...
        InlineKeyboardButton(
            text="Bot Info", callback_data="DevBot_"
        ),
    ],
    [
...
def DevBot_about_callback(update: Update, context: CallbackContext):
    first_name = update.effective_user.first_name
    query = update.callback_query
    if query.data == "DevBot_":
        query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                   [
                     InlineKeyboardButton(text="Support", url="t.me/DevBot_Support"),
                     InlineKeyboardButton(text="News", url="t.me/DevBot_Updates"),
                   ],
                   [
                    InlineKeyboardButton(text="Back", callback_data="DevBot_back")
                   ]
                ]
            ),
        )
    elif query.data == "DevBot_back":
        query.message.edit_text(
                PM_START_TEXT.format(
                        escape_markdown(first_name), escape_markdown(context.bot.first_name)),
                reply_markup=InlineKeyboardMarkup([
...
def main():
    ...
    about_callback_handler = CallbackQueryHandler(DevBot_about_callback, pattern=r"DevBot_", run_async=True)
